...
<head>
<meta charset="UTF-8">
<title>WASSUP- 리뷰 목록</title>
<script src="http://code.jquery.com/jquery-3.5.1.min.js"></script>

<script>
	$(document).ready(function(){
		<c:forEach items="${rList }" var="review">
		console.log(${review.orderCode});
		console.log($(".order").is("#"+${review.orderCode}));
		console.log($("#"+${review.orderCode}).children().last());
		var input = $("#"+${review.orderCode}).children().last().children();
		input.attr('value','리뷰 삭제');
		input.attr('onclick','location.href="/buyer/reviewDelete?orderCode=${review.orderCode}"');
		</c:forEach>
	});
            ...
            <div class="content_center">
                ...
                <table class="table table-hover">
                    <thead style="border: 1px solid white">
                     <tr height="80px;"><th>주문 번호</th><th>주문 일자</th><th>푸드트럭 명</th><th>금액</th><th>주문 현황</th><th>리뷰 버튼</th></tr>
                    </thead>
				<c:forEach items="${oList }" var="order">
					<tr height="50px;" class="order" id=${order.receiptCode } ><td>${order.receiptCode }</td><td>${order.orderDate }</td><td>${order.truckName }</td><td>${order.price }</td><td>${order.orderStatus }</td>
					<td><input type="button" value="리뷰 작성" class="btn" onclick="location.href='/buyer/reviewWrite?sCode=${order.sCode}&&receiptCode=${order.receiptCode }'"></td>
